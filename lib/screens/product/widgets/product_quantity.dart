...
class ProductQuantity extends StatelessWidget with ShapeMixin, ProductMixin {
  ...
  Widget build(BuildContext context) {
    ...
    int? max = b2bkingFilterQuantity(context, stockQty != null && stockQty > 0 ? stockQty : CirillaQuantity.maxQty, product ?? Product(), "max");
    int min = b2bkingFilterQuantity(context, 1, product ?? Product(), "min") ?? 1;
    int step = b2bkingFilterQuantity(context, 1, product ?? Product(), "step") ?? 1;
